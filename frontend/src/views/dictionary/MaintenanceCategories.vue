<template>
  <div class="maintenance-categories-container">
    <!-- 页面标题 -->
    <div class="page-header">
      <h1>运维维护类别管理</h1>
      <div class="header-actions">
        <button @click="refreshData" class="btn btn-secondary">🔄 刷新</button>
        <button @click="showCreateDialog" class="btn btn-primary">➕ 新增类别</button>
      </div>
    </div>
    ...
    <div class="search-form">
      <div class="search-row">
        <div class="form-group">
          <label>关键词</label>
          <input v-model="searchParams.keyword" placeholder="搜索类别名称或编码" />
        </div>
        <div class="form-group">
          <label>状态</label>
    ...
    <div class="table-container">
      ...
      <table v-else class="data-table">
        ...
        <tbody>
          <tr v-for="(item, index) in categoriesList" :key="item.id">
            <td class="row-number">{{ index + 1 }}</td>
            <td class="category-code">{{ item.code }}</td>
      ...
      <div v-if="!loading && categoriesList.length === 0" class="no-data">
        <p>暂无数据</p>
      </div>
    </div>
      ...
      <div class="modal-dialog" @click.stop>
        ...
        <div class="modal-body">
          <form @submit.prevent="saveItem" class="form">
            ...
            <div class="form-group">
              <label class="required">类别编码</label>
              <input
...
const categoriesList = ref<DictItem[]>([])
...
// 计算属性
const dialogTitle = computed(() => isEdit.value ? '编辑运维维护类别' : '新增运维维护类别')

const parentOptions = computed(() => {
  return categoriesList.value.filter(item => 
    item.id !== formData.id && // 排除自己
    !item.parent_id // 只显示顶级类别作为父级选项
  )
})
...
const getParentName = (parentId: number | null | undefined) => {
  if (!parentId) return '-'
  const parent = categoriesList.value.find(item => item.id === parentId)
...
const loadData = async () => {
  loading.value = true
  try {
    console.log('🔄 开始加载运维维护类别数据...')
    const response = await dictionaryApi.getMaintenanceCategories()
    ...
    console.error('❌ 加载运维维护类别失败:', error)
    categoriesList.value = []
    // 显示用户友好的错误信息
...
const searchData = () => {
  // 这里可以根据搜索参数过滤数据
  let filteredList = [...categoriesList.value]
...
// 显示编辑对话框
const showEditDialog = (item: DictItem) => {
  isEdit.value = true
...
const saveItem = async () => {
  ...
  try {
    ...
    if (isEdit.value) {
      await dictionaryApi.updateMaintenanceCategory(formData.id, data)
    } else {
      await dictionaryApi.createMaintenanceCategory(data)
    }
...
// 切换状态
const toggleStatus = async (item: DictItem) => {
  try {
    await dictionaryApi.updateMaintenanceCategory(item.id, {
      ...item,
      is_active: !item.is_active
    })
    await loadData()
...
const confirmDelete = (item: DictItem) => {
  if (confirm(`确定要删除类别"${item.name}"吗？`)) {
    deleteItem(item)
  }
}
...
const deleteItem = async (item: DictItem) => {
  try {
    await dictionaryApi.deleteMaintenanceCategory(item.id)
    await loadData()
  } catch (error) {
...
<style scoped>
.maintenance-categories-container {
  padding: 20px;
  max-width: 1200px;
  margin: 0 auto;
}
