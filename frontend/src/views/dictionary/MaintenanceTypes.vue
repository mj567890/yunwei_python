  ...
  <div class="maintenance-types-container">
    ...
    <div class="page-header">
      ...
      <div class="header-actions">
        ...
        <button @click="showCreateDialog" class="btn btn-primary">➕ 新增类型</button>
      </div>
    </div>
    ...
    <div class="search-form">
      <div class="search-row">
        <div class="form-group">
          <label>关键词</label>
          <input v-model="searchParams.keyword" placeholder="搜索类型名称或编码" />
        </div>
        <div class="form-group">
          <label>状态</label>
    ...
    <div class="table-container">
      ...
      <table v-else class="data-table">
        ...
        <tbody>
          <tr v-for="(item, index) in typesList" :key="item.id">
            <td class="row-number">{{ index + 1 }}</td>
            <td class="type-code">{{ item.code }}</td>
            <td class="type-name">
      ...
      <div v-if="!loading && typesList.length === 0" class="no-data">
        <p>暂无数据</p>
      </div>
    </div>
      ...
      <div class="modal-dialog" @click.stop>
        ...
        <div class="modal-body">
          <form @submit.prevent="saveItem" class="form">
            ...
            <div class="form-group">
              <label class="required">类型编码</label>
              <input
...
// 计算属性
const dialogTitle = computed(() => isEdit.value ? '编辑运维记录类型' : '新增运维记录类型')

const parentOptions = computed(() => {
  return typesList.value.filter(item => 
    item.id !== formData.id && // 排除自己
    !item.parent_id // 只显示顶级类型作为父级选项
  )
})
...
const getParentName = (parentId: number | null | undefined) => {
  if (!parentId) return '-'
  const parent = typesList.value.find(item => item.id === parentId)
...
const loadData = async () => {
  loading.value = true
  try {
    console.log('🔄 开始加载运维记录类型数据...')
    const response = await dictionaryApi.getMaintenanceTypes()
    ...
    console.error('❌ 加载运维记录类型失败:', error)
    typesList.value = []
    // 显示用户友好的错误信息
...
const searchData = () => {
  // 这里可以根据搜索参数过滤数据
  let filteredList = [...typesList.value]
...
const saveItem = async () => {
  ...
  try {
    ...
    if (isEdit.value) {
      await dictionaryApi.updateMaintenanceType(formData.id, data)
    } else {
      await dictionaryApi.createMaintenanceType(data)
    }
...
const toggleStatus = async (item: DictItem) => {
  try {
    await dictionaryApi.updateMaintenanceType(item.id, {
      ...item,
      is_active: !item.is_active
    })
    await loadData()
...
const confirmDelete = (item: DictItem) => {
  if (confirm(`确定要删除类型"${item.name}"吗？`)) {
    deleteItem(item)
  }
}
...
const deleteItem = async (item: DictItem) => {
  try {
    await dictionaryApi.deleteMaintenanceType(item.id)
    await loadData()
  } catch (error) {
